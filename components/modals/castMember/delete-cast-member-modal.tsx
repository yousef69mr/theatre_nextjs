...
export const DeleteCastMemberModal = () => {
  ...
  const isModalOpen = isOpen && type === "deleteCastMember";
  const { castMember } = data;
  ...
  const onClick = async () => {
    try {
      ...
      await fetch(PUBLIC_DOMAIN.concat(`/api/cast-members/${castMember.id}`), {
        method: "DELETE",
      });
        ...
        <DialogHeader className="pt-8 px-6">
          ...
          <DialogDescription className="text-center text-zinc-500">
            ...
            <span className="text-primary font-semibold">
              ...
              {t("actions.linkTo", {
                ns: "common",
                instance: castMember?.actor.name,
                to: t(`UserRole.${castMember?.role}`, { ns: "common" }),
              })}
            </span>
            <br />
