...
import { ActorInPlayType, ActorType, FestivalType, UserType } from "@/types";
...
import {
  createUserActorLinkRequest,
  getRemainingUserActorLinkRequest,
} from "@/lib/api-calls/actions/user-actor-link";
import { isAdmin } from "@/lib/auth";
import { useCurrentRole } from "@/hooks/use-current-role";
import { UserRole } from "@prisma/client";

interface LinkUserActorFormProps extends HtmlHTMLAttributes<HTMLElement> {
  initialData: {
    actorId?: string;
    userId?: string;
  } | null;
...
type LinkUserActorFormValues = Zod.infer<typeof userActorLinkSchema>;

const LinkUserActorForm: FC<LinkUserActorFormProps> = (props) => {
  ...
  const [actors, setActors] = useState<ActorType[] | undefined>();
  ...
  const form = useForm<LinkUserActorFormValues>({
    resolver: zodResolver(userActorLinkSchema),
    defaultValues: {
      ...initialData,
    },
  });

  const onSubmit = async (values: LinkUserActorFormValues) => {
    setIsLoading(true);
                  ...
                  <PopoverTrigger asChild>
                    <FormControl>
                      ...
                      >
                        {field.value
                          ? `${
                              actors?.find((actor) => actor.id === field.value)
                                ?.name
                            } ${
                              actors?.find((actor) => actor.id === field.value)
                                ?.nickname
...
export default LinkUserActorForm;
